...
export const Skills = ({ data, mySkills, setSkills }) => {
  ...
  //   setSkills(mySkills.pop(skill.target.outerText))
  // }
        ...
        {mySkills && mySkills.map((mySkill) => (
          <div className="grid grid-cols-3">
            <div className="p-2 ml-5 mt-5">
              <span id="badge-dismiss-purple" className=" py-2 px-3 mr-3 text-sm font-medium text-purple-800 bg-purple-100 rounded-full dark:bg-purple-200 dark:text-purple-800">
                {mySkill}
                <button type="button" class=" items-center p-0.5 ml-2 text-sm text-purple-400 bg-transparent rounded-sm hover:bg-purple-200 hover:text-purple-900 dark:hover:bg-purple-300 dark:hover:text-purple-900" data-dismiss-target="#badge-dismiss-purple" aria-label="Remove">
